...
/root/staticperl mkapp BubbleBreaker --boot /root/.staticperl/perl/bin/bubble-breaker.pl --incglob 'SDL*' --strip \
...
--staticlib vga --staticlib x86 --staticlib SDL --staticlib SDLmain \
...
# see also: http://git.savannah.gnu.org/gitweb/?p=freedink.git;a=blob;f=doc/static-build.txt
